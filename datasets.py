...
def CUBfs(use_hd=True):
    ...
    all_transforms = torch.nn.Sequential(transforms.Resize(int(1.15*image_size)),#transforms.Resize([int(1.15*image_size), int(1.15*image_size)]), 
                                         transforms.CenterCrop(image_size), 
                                         norm) if args.sample_aug == 1 else torch.nn.Sequential(transforms.RandomResizedCrop([image_size, image_size], scale=(0.14,1)), norm)
    if args.episodic:

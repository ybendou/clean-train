...
def miniImageNet(use_hd = True):
    ...
    if args.model=='amdimnet':
        #norm = transforms.Normalize(np.array([x / 255.0 for x in [125.3, 123.0, 113.9]]), np.array([x / 255.0 for x in [63.0, 62.1, 66.7]]))
        norm = transforms.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225])
        train_transforms = torch.nn.Sequential(transforms.RandomResizedCrop(128), transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4), transforms.RandomHorizontalFlip(), norm)
        all_transforms = torch.nn.Sequential(transforms.Resize(146, interpolation=3), transforms.CenterCrop(128), norm)
        if args.episodic:

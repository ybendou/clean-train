...
for i in tqdm(range(len(datasets[0]))):
    ...
    img = transforms.ToTensor()(np.array(Image.open(img_path).convert('RGB')))
    ...
    for K_resize in [84, 92, 100, 110, 128, 164, 184]:
        for _ in range(args.n_augmentation):
            ...
            elt, params = crop_resize_rescale_image(img, augmentations, K_resize)
            with torch.no_grad():
                elt = elt.to(args.device)
                _, features = model(elt.unsqueeze(0))
            # compute distance of feature to class centroid
            distance = torch.norm(features.squeeze(0)-class_centroid, p=2, dim=0)

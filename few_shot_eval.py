...
def eval_few_shot(train_features, val_features, novel_features, val_run_classes, val_run_indices, novel_run_classes, novel_run_indices, n_shots, T):
    return ncm(train_features, val_features, val_run_classes, val_run_indices, n_shots, T), ncm(train_features, novel_features, novel_run_classes, novel_run_indices, n_shots, T)

def update_few_shot_meta_data(model, T, L, train_clean, novel_loader, val_loader, few_shot_meta_data):
    ...
    for i in range(len(args.n_shots)):
        res.append(evaluate_shot(i, train_features, val_features, novel_features, few_shot_meta_data, T, L, model = model))

    return res

def evaluate_shot(index, train_features, val_features, novel_features, few_shot_meta_data, T, L, model = None):
    (val_acc, val_conf), (novel_acc, novel_conf) = eval_few_shot(train_features, val_features, novel_features, few_shot_meta_data["val_run_classes"][index], few_shot_meta_data["val_run_indices"][index], few_shot_meta_data["novel_run_classes"][index], few_shot_meta_data["novel_run_indices"][index], args.n_shots[index], T)
        ...
        if val_acc > few_shot_meta_data["best_val_acc_ever"][index]:
            ...
            if args.save_model != "":
                if len(args.devices) == 1:
                    ...
                    torch.save(T.state_dict(), args.save_model +"T"+ str(args.n_shots[index]))
                    torch.save(L.state_dict(), args.save_model +"L"+ str(args.n_shots[index]))
